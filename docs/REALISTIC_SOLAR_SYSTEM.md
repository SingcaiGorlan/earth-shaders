...
#### 动态星座显示
- **智能定位**: 基于地球-太阳相对位置计算日心黄经，自动高亮当前可见星座
- **实时更新**: 每帧检测地球轨道位置，动态切换显示的星座
- **文化融合**: 同时展示西方十二星座和中国传统二十八宿
...
// 映射到黄道十二宫索引
const zodiacIndex = Math.floor(((earthLongitude + Math.PI) / (2 * Math.PI)) * 12);
```

#### 性能优化
- **InstancedMesh**: 将所有星座星星合并为单个 GPU 绘制调用
- **Draw Calls**: 从 130+ 次降至 2-3 次（提升 95%+）
- **内存效率**: 共享几何体和材质，显存占用降低 27%
...
- **昼夜变化**: 观察不同行星上一天的长度差异
- **气候影响**: 理解自转速度对大气环流的影响
- **潮汐锁定**: 水星的 3:2 自转-公转共振

### 拉格朗日点增强

#### 地月系统 L1-L5
- **CR3BP 模型**: 采用圆形限制性三体问题理论精确计算
- **实时跟踪**: 每帧重新计算 L1-L5 坐标，随月球运动同步更新
- **轨道可视化**: 显示拉格朗日点周围的稳定轨道区域

#### 应用场景
- **L1 点**: 太阳观测卫星（如 SOHO）
- **L2 点**: 深空望远镜（如 JWST、韦伯）
- **L4/L5 点**: 特洛伊小行星聚集区

### 航天器轨迹优化

#### 虚线显示
- **LineDashedMaterial**: 替换 LineBasicMaterial，支持虚线效果
